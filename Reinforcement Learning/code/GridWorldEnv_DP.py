...
class GridWorldEnv:
    ...
    # reset state
    def reset(self, ir=0, ic=0, random=True):
        if random:
...
class GWE_Trap(GridWorldEnv):  # One goal, one trap
    def __init__(self, reward=0, trap_penalty=-10, step_penalty=-1):
        super().__init__()
        self.nr = 5
        self.nc = 5
        self.er = 4
    ...
    def step(self, action):
        ...
        tr, tc, valid = self.next_state(self.sr, self.sc, action)
